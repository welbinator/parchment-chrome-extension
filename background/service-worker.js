...
Keep it concise and practical. Do not include filler phrases like "In this video..." or "The creator says...". Just the useful information.`;
  ...
  try {
    if (settings.aiProvider === 'openai') {
      ...
      return await callGemini(settings.aiApiKey, settings.aiModel || 'gemini-2.5-flash-preview-04-17', prompt);
    }
...
${rawData.html}`;
  try {
    ...
    if (settings.aiProvider === 'openai') {
      ...
      text = await callGemini(settings.aiApiKey, settings.aiModel || 'gemini-2.5-flash-preview-04-17', prompt);
    }
